...
@CommandLine.Command(
		...
		description = "Adjust population outside a shape file"
)
public class AdjustPopulationForCutout implements MATSimAppCommand {

	private static final Logger log = LogManager.getLogger(AdjustPopulationForCutout.class);

	@CommandLine.Parameters(arity = "1", paramLabel = "INPUT", description = "Input population xml", defaultValue = "scenarios/input/duesseldorf-" + VERSION + "-25pct.plans.xml.gz")
	private List<Path> input;

	@CommandLine.Option(names = "--network", description = "Network xml", defaultValue = "scenarios/input/duesseldorf-" + VERSION + "-network.xml.gz")
	private Path networkPath;

	@CommandLine.Option(names = "--output", description = "Output population xml", defaultValue = "scenarios/input/duesseldorf-" + VERSION + "-25pct.adj-plans.xml.gz")
	private Path output;

	@CommandLine.Option(names = "--scale", description = "Additional scale for the length", defaultValue = "1.15")
	...
	public static void main(String[] args) {
		new AdjustPopulationForCutout().execute(args);
	}
