...
public class ShortTripsGenerator {
	...
	public static void main(String[] args) {
		...
		if (args.length == 0) {
			plans = "C:\\Users\\cluac\\MATSimScenarios\\Dusseldorf\\Scenario\\original_plans.xml.gz";
			shapeFile = "C:\\Users\\cluac\\MATSimScenarios\\Dusseldorf\\DusseldorfAreaShapeFile\\duesseldorf-area.shp";
			outputFolder = "C:\\Users\\cluac\\MATSimScenarios\\Dusseldorf\\Scenario";
			missingTrips = 129300;
			range = 1000;
		} else {
			plans = args[0];
...
//        };
		...
		PopulationUtils.writePopulation(shortTripsGenerator.getPopulation(),
				outputFolder + "\\plans_with_short_trips.xml.gz");
				...
				if (markedActivities.size() > 0) {
					...
					for (TripStructureUtils.Trip trip : TripStructureUtils.getTrips(plan.getPlanElements())) {
						...
						if (markedActivities.contains(originActivity)) {
							...
							double duration = Math.min(this.maxShortRangeActivityDuration,
							...
							Activity shortDistanceRangeActivity = population.getFactory().createActivityFromCoord(
									"other", getShortDistanceCoordinate(trip.getOriginActivity().getCoord(), range));
